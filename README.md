...
3. **Download Terraform binary.** Go to [Terraform site](https://www.terraform.io/downloads.html) and download the latest version of Terraform. (You can use Algo wrapper in order to let Algo download the latest version of Terraform, see instructions bellow). `cd` into the `algo-master` directory where you unzipped Algo, then run:
